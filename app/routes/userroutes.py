...
from app.schemas.schema import UserCreate, UserResponse , UserUpadte ,UpdateRoleSchema,PasswordChange, LoginRequest, UserVerify
from typing import List
from app.db.session import get_db
from app.services.auth import sign_up, login, get_all_user, get_user_by_id, update_user, delete_user, role_change, verify_user_email
...
def verify_email(data:UserVerify, db:Session = Depends(get_db
    ...
    user = verify_user_email(db, data.email, data.token)
...
@router.post("/login")
@limiter.limit("100/minute")
def login_route( request: Request,background_tasks: BackgroundTasks, user: LoginRequest, db: Session = Depends(get_db)):
    return login(db, user.email, user.password, background_tasks)
...
def change_password_route( request: Request,background_tasks: BackgroundTasks ,user_id:int,
    data: PasswordChange,
    ...
    current_user: User = Depends(get_current_user)
     ...
     return change_password(db, current_user.id, data.old_password, data.new_password, background_tasks)


# role change only admin can change the role 
@router.patch("/{user_id}/role", response_model=UserResponse)
@limiter.limit("5/minute")
def patch_user_role( request: Request,background_tasks: BackgroundTasks, user_id:int, data:UpdateRoleSchema, db:Session=Depends(get_db), current_user:User= Depends(require_roles(["admin"]))):
    return role_change(db,current_user.id,user_id,data.role, background_tasks)